...
<body>
    ...
    <form action="{{ route('product.order', ['idProduct' => $product->id]) }}" method="get">
        @csrf
        ...
        <main class="flex grid grid-cols-3 gap-4">
            ...
            <section id="productDescriptions" class="">
                            ...
                            @foreach ($variant->availableSizes as $availableSize)
                                <div class="ml-4 mt-2 size-container" data-color-id="{{ $variant->color_id }}"
                                    ...
                                    <input type="radio" name="size_name"
                                        ...
                                        data-stock="{{ $availableSize->stock }}">
                ...
                @if (Session::get('email') != null)
                    <button
                        class="bg-black text-white hover:bg-gray-700 hover:text-white rounded mt-4 py-2 px-4 w-2/3 buy-now-button">
                        <div class="out-of-stock-message hidden">


                            <h1 class="text-center p-3 text-5xl font-semibold"> Out of Stock</h1>
                        </div>
                        BUY NOW
                        @csrf
                    </button>
                @else
                    <button
                        class="bg-black text-white disabled hover:bg-gray-700 hover:text-white rounded mt-4 py-2 px-4 w-2/3" disabled>
                        <div class="out-of-stock-message hidden">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('input[name="size_name"]').change(function() {
                ...
                if ($('input[name="selected_color"]:checked').length > 0 && $(
                        'input[name="size_name"]:checked').length > 0) {
                    var selectedStock = $('input[name="size_name"]:checked').data('stock');

                    if (selectedStock > 0) {
                        // Enable the "Buy Now" button
                        $('button.buy-now-button').prop('disabled', false);
                        // Hide the "Out of Stock" message
                        $('.out-of-stock-message').addClass('hidden');
                    } else {
                        // Disable the "Buy Now" button
                        $('button.buy-now-button').prop('disabled', true);
                        // Show the "Out of Stock" message
                        $('.out-of-stock-message').removeClass('hidden');
                    }
                    ...
                    $('label[for="qty"]').show();
                    $('#qty').show();
                    $('button.buy-now-button').show();
                } else {
                    // Hide the "Qty" input and "Buy Now" button if size is not selected
                    $('label[for="qty"]').hide();
                    $('#qty').hide();
                    $('button.buy-now-button').hide();
                    // Hide the "Out of Stock" message
                    $('.out-of-stock-message').addClass('hidden');
                }
            });
