...
class CustomerController extends Controller
{
    ...
    public function orderHistory($idLogin)
    {

        $orderHistories = Transaction::with('productTransaction', 'productVariantTransactions')->where('user_id', $idLogin)->paginate(10);
        return view('orderHistory', compact('orderHistories'));
    }
    ...
    public function showBestSeller()
    {
        ...
        $bestSellerIds = Transaction::select('product_id')
            ->groupBy('product_id')
            ->orderByDesc(DB::raw('SUM(total_price)'))
            // ->with('productTransaction', 'productVariantTransactions')
            ->limit(5)
            ->pluck('product_id');
        ...
        $bestSellers = Product::whereIn('id', $bestSellerIds)
            ->with('variants.productFiles')
            ->get();
 // dd($bestSellerIds, $bestSellers);





        return view('bestSellerView', compact('bestSellers'));
    }
}
